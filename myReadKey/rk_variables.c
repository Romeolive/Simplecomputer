...
/* Стрелки обычно: ESC [ A/B/C/D
   ...
   В некоторых терминалах: ESC O P/Q/... — но нам нужны F5/F6, оставим xterm-вариант.
*/
int
  ...
  if (ch == 27) /* ESC */
    {
      ...
      for (int i = 0; i < 7; i++)
        {
          unsigned char t;
          if (read_byte_timeout(&t, 20) != 0) break;
          seq[n++] = t;
        }
      ...
      if (n == 2 && seq[0] == '[')
        {
          if (seq[1] == 'A') { *key = KEY_UP; return 0; }
          if (seq[1] == 'B') { *key = KEY_DOWN; return 0; }
          if (seq[1] == 'C') { *key = KEY_RIGHT; return 0; }
          if (seq[1] == 'D') { *key = KEY_LEFT; return 0; }
        }

      /* F5 / F6: [15~ / [17~ */
      if (n == 4 && seq[0] == '[' && seq[3] == '~')
        {
          if (seq[1] == '1' && seq[2] == '5') { *key = KEY_F5; return 0; }
          if (seq[1] == '1' && seq[2] == '7') { *key = KEY_F6; return 0; }
        }
  ...
  for (;;)
    {
      ...
      if (ch == '+' || ch == '-')
        {
          if (pos != 0) continue; /* знак только первым */
          buf[pos++] = (char)ch;
          continue;
        }
